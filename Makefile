...
typecheck: ## Run type checking with mypy
	uv run mypy openai_tts_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r openai_tts_tool -c pyproject.toml
...
run: ## Run openai-tts-tool (usage: make run ARGS="...")
...
uninstall-global: ## Uninstall global installation
	uv tool uninstall openai-tts-tool
